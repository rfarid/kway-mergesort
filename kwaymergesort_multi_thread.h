...
template <class T>
void KwayMergeSort<T>::dispProgress(float progress) {
    int barWidth = 70;

    std::cout << "[";
    int pos = barWidth * progress;
    // std::string bar=std::string(pos, '=');
    // std::string empty_space=std::string(barWidth-pos, ' ');
    // std::cout << "[" << bar << ">" << empty_space << "] " << int(progress * 100.0) << " %\r";
...
void KwayMergeSort<T>::DivideAndSort() {
    ...
    unsigned int total = 0;  // track the number of bytes consumed so far totally.
...
void KwayMergeSort<T>::Merge() {
    ...
    if (_tempFileUsed == false) {
        // Showing the final progress bar
        dispProgress(1.0);
        // Moving to next line
        cout<<endl;
        return;
...
template <class T>
void KwayMergeSort<T>::expensiveFunc() {
    usleep(_delaySec);
}
